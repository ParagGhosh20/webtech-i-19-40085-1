...
function showStudent($username){
	$conn = db_conn();
	$selectQuery = "SELECT * FROM `votersinfo` where USERNAME = '$username'";
...
function addSignupInfo($data){
	$conn = db_conn();
    $selectQuery = "INSERT into votersinfo (NAME, EMAIL, BIRTH,USERNAME,PASSWORD,GENDER)
...
function updateStudent($username, $data){
    $conn = db_conn();
    $selectQuery = "UPDATE votersinfo set NAME = ?, EMAIL = ?, BIRTH = ? where USERNAME = '$username'";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
...
function updatePasswordStudent($username, $password){
    $conn = db_conn();
    $selectQuery = "UPDATE votersinfo set PASSWORD = '$password' where USERNAME = '$username'";
    try{
          $stmt = $conn->query($selectQuery);
